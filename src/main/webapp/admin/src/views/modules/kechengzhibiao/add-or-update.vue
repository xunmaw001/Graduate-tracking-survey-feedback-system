<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="项目" prop="xiangmu">
          <el-input v-model="ruleForm.xiangmu" 
              placeholder="项目" clearable  :readonly="ro.xiangmu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="项目" prop="xiangmu">
              <el-input v-model="ruleForm.xiangmu" 
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="评审内容" prop="pingshenneirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="评审内容"
                  v-model="ruleForm.pingshenneirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.pingshenneirong" label="评审内容" prop="pingshenneirong">
                    <span>{{ruleForm.pingshenneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="评审标准" prop="pingshenbiaozhun">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="评审标准"
                  v-model="ruleForm.pingshenbiaozhun" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.pingshenbiaozhun" label="评审标准" prop="pingshenbiaozhun">
                    <span>{{ruleForm.pingshenbiaozhun}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
    ...
    return {
      ...
      ro:{
	xiangmu : false,
	pingshenneirong : false,
	pingshenbiaozhun : false,
	fenshu : false,
      },
      ruleForm: {
        xiangmu: '',
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xiangmu = true;
            continue;
          }
          if(o=='pingshenneirong'){
            this.ruleForm.pingshenneirong = obj[o];
	    this.ro.pingshenneirong = true;
            continue;
          }
          if(o=='pingshenbiaozhun'){
            this.ruleForm.pingshenbiaozhun = obj[o];
	    this.ro.pingshenbiaozhun = true;
            continue;
          }
          if(o=='fenshu'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `kechengzhibiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.kechengzhibiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
